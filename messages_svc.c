...
int
_set_value_rpc_1 (tupla  *argp, void *result, struct svc_req *rqstp)
{
...
int
_modify_value_rpc_1 (tupla  *argp, void *result, struct svc_req *rqstp)
{
...
int
_copy_key_rpc_1 (double_key  *argp, void *result, struct svc_req *rqstp)
{
	return (copy_key_rpc_1_svc(*argp, result, rqstp));
}

static void
messages_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		tupla set_value_rpc_1_arg;
		int get_value_rpc_1_arg;
		tupla modify_value_rpc_1_arg;
		int delete_key_rpc_1_arg;
		int exist_rpc_1_arg;
		double_key copy_key_rpc_1_arg;
	} argument;
	union {
		int init_rpc_1_res;
		int set_value_rpc_1_res;
		tupla get_value_rpc_1_res;
		int modify_value_rpc_1_res;
		int delete_key_rpc_1_res;
		int exist_rpc_1_res;
		int copy_key_rpc_1_res;
	} result;
	bool_t retval;
	...
	case get_value_rpc:
		...
		_xdr_result = (xdrproc_t) xdr_tupla;
	...
	case copy_key_rpc:
		_xdr_argument = (xdrproc_t) xdr_double_key;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (bool_t (*) (char *, void *,  struct svc_req *))_copy_key_rpc_1;
		break;
	...
	if (!messages_1_freeresult (transp, _xdr_result, (caddr_t) &result))
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, MESSAGES, FIRST_VER, messages_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (MESSAGES, FIRST_VER, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, MESSAGES, FIRST_VER, messages_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (MESSAGES, FIRST_VER, tcp).");
		exit(1);
	}
